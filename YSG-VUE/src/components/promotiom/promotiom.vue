          ...
          <scroller
              ...
              <ul>
                  <li v-for="data in dataList" @click="goDetail(data.id)">
                      ...
                      <div class="s-right">
                          <img :src='data.pic' style="height:1.8rem;"/>
                          <!-- <div class="s-tag" v-show="data.tagName"><span>{{data.tagName}}</span></div> -->
                      </div>
                      <div style="display:block;width:100%;float:left;margin-top:5px;">
                          <span style="float: left;font-size: .28rem;color: #f0c366;">
                              {{new Date(data.updatetime*1000).getFullYear()+'-'+(new Date(data.updatetime*1000).getMonth() + 1)+'-'+new Date(data.updatetime*1000).getDate()}}
                            </span>
                          <div style="float: right;border-radius: 3px;color: #666;border: 1px solid #eee;padding: 3px .2rem;height: 0.58rem;" v-show="data.tagName">
		...
		methods: {
			...
			goDetail(id) {
                ...
                if(!data.outLink){
                    ...
                    this.$router.push({path:'/promotionOpen',query:{url:data.outLink,title:data.title}});
                }
			},
