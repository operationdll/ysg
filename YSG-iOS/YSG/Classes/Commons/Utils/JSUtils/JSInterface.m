...
const NSString *EventHandler = @"JSInterface";
@implementation JSInterface

static JSInterface * _handler= nil;
+ (instancetype)shareInstance{
    
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _handler =[JSInterface new];
...
- (NSString *)getJsString{
    ...
    NSString *path =[[NSBundle bundleForClass:[self class]] pathForResource:@"JSInterface" ofType:@"js"];
...
+ (void)getInject:(WKWebView *)webView{
    ...
    [JSInterface shareInstance].webView = webView;
    ...
    if ([message.name isEqualToString:EventHandler]) {
        ...
        if (callBackName) {
            ...
            [self interactWitMethodName:methodName params:params :^(id response) {
                ...
                NSString *js = [NSString stringWithFormat:@"JSInterface.callBack('%@',%@);",callBackName,response];
